...
class SampleSetup
{
    ...
    public function searchForKey($keyOfArray, $array)
    {
    ...
    private function initInstallmentForCustom(InstallmentRequest $installmentRequest, InstallmentsResponse $installmentResponse)
    {
        ...
        } elseif (empty($installmentRequest->getBinNo()) || empty($installmentRequest->getTotalAmount()) || empty($installmentRequest->getTicketId())) {
            $installmentResponse->setError('Request Body variables can not be null !');

            return $installmentResponse;
        } elseif (!EncryptionUtil::verifyBexSign($installmentRequest->getTicketId(), $installmentRequest->getSignature())) {
        ...
        if (isset($key)) {
            ...
            for ($i = 0; $i < count($customInstallmentData); $i++) {
                ...
                $out = ['numberOfInstallment' => $installment->getNumberOfInstallment(), 'installmentAmount' => $installment->getInstallmentAmount(),
    ...
    private function prepareVposConfig($bankCode)
    {
        ...
        if (Banks::AKBANK == $bankCode) {
        ...
        } elseif (Banks::VAKIFBANK == $bankCode) {
        ...
        } elseif (Banks::ALBARAKA == $bankCode) {
        ...
        } elseif (Banks::BANKASYA == $bankCode) {
        ...
        } elseif (Banks::DENIZBANK == $bankCode) {
        ...
        } elseif (Banks::FINANSBANK == $bankCode) {
            $vposConfig->setVposUserId('bkmapi');
            $vposConfig->setVposPassword('TEST1234');
            ...
            $vposConfig->setBankIndicator($bankCode);
            $vposConfig->setServiceUrl('http://srvirt01:7200/VPServlet');
        } elseif (Banks::HALKBANK == $bankCode) {
        ...
        } elseif (Banks::KUVEYTTURK == $bankCode) {
        ...
        } elseif (Banks::ODEABANK == $bankCode) {
            $vposConfig->setVposUserId('600218');
            $vposConfig->setVposPassword('123qweASD');
            $vposConfig->addExtra('terminalprovuserid', 'PROVAUT');
            $vposConfig->addExtra('terminalmerchantid', '7000679');
            ...
            $vposConfig->setBankIndicator($bankCode);
            $vposConfig->setServiceUrl('http://srvirt01:7200/VPServlet');
        } elseif (Banks::SEKERBANK == $bankCode) {
            $vposConfig->setVposUserId('600218');
            $vposConfig->setVposPassword('123qweASD');
            $vposConfig->addExtra('terminalprovuserid', 'PROVAUT');
            $vposConfig->addExtra('terminalmerchantid', '7000679');
        ...
        } elseif (Banks::ZIRAATBANK == $bankCode) {
        ...
        $vps_jsn = ['vposUserId' => $vposConfig->getVposUserId(), 'vposPassword' => $vposConfig->getVposPassword(), 'extra' => $vposConfig->getExtra(), 'bankIndicator' => $vposConfig->getBankIndicator(), 'serviceUrl' => $vposConfig->getServiceUrl()];
